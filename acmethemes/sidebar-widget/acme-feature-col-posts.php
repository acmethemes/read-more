...
if ( ! class_exists( 'Read_more_feature_posts_col' ) ) {
    ...
    class Read_more_feature_posts_col extends WP_Widget {
        ...
        public function form( $instance ) {
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title', 'read-more' ); ?>:</label>
                <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo $title; ?>" />
            </p>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'read_more_cat_id' ) ); ?>" class="update-nag"><?php _e('Select Category', 'read-more'); ?></label>
                <?php
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'read_more_post_col_first_featured_image_layout' ) ); ?>">
                    <?php _e( 'First Featured Post Image', 'read-more' ); ?>
                </label>
                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'read_more_post_col_first_featured_image_layout' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'read_more_post_col_first_featured_image_layout' ) ); ?>">
                    <?php
                    foreach( $choices as $key => $read_more_column_array ){
                        echo ' <option value="'.$key.'"'.selected( esc_attr( $read_more_post_col_first_featured_image_layout ), $key, 0).'>'.$read_more_column_array.'</option>';
                    }
                    ?>
                </select>
            </p>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'read_more_post_col_first_featured_content_length' ) ); ?>"><?php _e( 'First Featured Post Content Length', 'read-more' ); ?>:</label>
                <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'read_more_post_col_first_featured_content_length' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'read_more_post_col_first_featured_content_length' ) ); ?>" type="number" value="<?php echo esc_attr( $read_more_post_col_first_featured_content_length ); ?>" />
            </p>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'read_more_post_col_normal_image_layout' ) ); ?>">
                    <?php _e( 'Normal Featured Post Image', 'read-more' ); ?>
                </label>
                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'read_more_post_col_normal_image_layout' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'read_more_post_col_normal_image_layout' ) ); ?>">
                    <?php
                    foreach( $choices as $key => $read_more_column_array ){
                        echo ' <option value="'.$key.'"'.selected( esc_attr( $read_more_post_col_normal_image_layout ), $key, 0).'>'.$read_more_column_array.'</option>';
                    }
                    ?>
                </select>
            </p>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'read_more_post_col_normal_content_length' ) ); ?>"><?php _e( 'Normal Featured Post Content Length', 'read-more' ); ?>:</label>
                <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'read_more_post_col_normal_content_length' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'read_more_post_col_normal_content_length' ) ); ?>" type="number" value="<?php echo esc_attr( $read_more_post_col_normal_content_length ); ?>" />
            </p>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'read_more_text' ) ); ?>"><?php _e( 'Read More Text', 'read-more' ); ?>:</label>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'button_text' ) ); ?>"><?php _e( 'Button Text', 'read-more' ); ?>:</label>
                <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'button_text' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'button_text' ) ); ?>" type="text" value="<?php echo esc_html( $button_text ); ?>" />
            </p>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'button_url' ) ); ?>"><?php _e( 'Button Link Url', 'read-more' ); ?>:</label>
                <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'button_url' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'button_url' ) ); ?>" type="text" value="<?php echo $button_url; ?>" />
            </p>
            <?php
        }
                    ...
                    if ( $the_query->have_posts() ):
                        ...
                        while( $the_query->have_posts() ):$the_query->the_post();
                            if( 'read-more-footer-full-width' == $read_more_sidebar_id ||
                                ...
                                ?>
                                <div class="blog-item <?php echo esc_attr( $inner_content ); ?>">
                                    <?php get_template_part( 'template-parts/content', 'recents' );?>
                                </div>
                                <?php
                                echo wp_kses_post( $i_1_end );
                                echo wp_kses_post( $i_2_end );
                            }
                            else{
                                $b_col = 'col-sm-12';
                                ?>
                        ...
                        <div class="at-btn-wrap col-sm-12">
                            <div class="btn-wrapper clearfix">
                                <div class="btn-icon-box">
                                    <div class="btn-data">
                                        <a class="btn-links" href="<?php echo esc_url( $button_url ); ?>">
